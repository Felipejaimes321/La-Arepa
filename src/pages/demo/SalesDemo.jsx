        ...
        <div className="w-full max-w-md">
          <div className="bg-white border border-green-300/60 rounded-2xl p-6 shadow-sm">
            ...
            <div className="bg-[#FAFAFA] border border-[#E5E5E5] rounded-xl p-4 font-mono text-xs space-y-1 mb-4">
              <div className="text-center text-[#D62B2B] font-bold text-sm mb-2">🫓 LA AREPA · CHORIZO & BEBIDA</div>
              ...
              <div className="border-t border-[#E5E5E5] my-2"/>
              <div className="flex justify-between font-bold text-[#D62B2B]"><span>TOTAL</span><span>${invoice.total.toLocaleString()}</span></div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={nueva} className="flex-1 bg-[#D62B2B] text-white font-bold py-3 rounded-xl hover:bg-[#B82222] transition-all text-sm">
                Nueva Venta
              </button>
            </div>
          </div>
    ...
    <div className="h-screen flex flex-col md:flex-row overflow-hidden" style={{background:'#FAFAFA'}}>
      <div className="flex-1 overflow-y-auto p-5 border-r border-[#E5E5E5]">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {cats.map(c => (
            <button key={c} onClick={() => setCat(c)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${cat===c?'bg-[#D62B2B] text-white':'bg-[#1A1A1A]/8 text-[#1A1A1A]/60 hover:bg-[#1A1A1A]/12'}`}>{c}</button>
          ))}
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {filtered.map(item => (
            <button key={item.id} onClick={() => addItem(item)} className="pos-btn bg-white border border-[#E5E5E5] rounded-xl p-4 text-left hover:border-[#D62B2B]/30 transition-all hover:-translate-y-0.5 group shadow-sm">
              ...
              <div className="text-[#D62B2B] font-display font-bold text-sm mt-1">${item.price.toLocaleString()}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="w-full md:w-80 bg-white flex flex-col border-t md:border-t-0 border-[#E5E5E5]" style={{maxHeight:'100vh'}}>
        <div className="p-4 border-b border-[#E5E5E5] flex items-center gap-2">
          <ShoppingCart size={18} className="text-[#D62B2B]"/>
          ...
          {cart.length === 0 ? (
            ...
            <div key={item.id} className="flex items-center gap-2 bg-[#FAFAFA] border border-[#E5E5E5] rounded-xl p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[#D62B2B] text-xs">${(item.price*item.qty).toLocaleString()}</div>
              </div>
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-[#E5E5E5] space-y-3">
            <input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Nombre del cliente (opcional)"
              className="w-full bg-[#FAFAFA] border border-[#E5E5E5] rounded-xl px-3 py-2 text-[#1A1A1A] text-sm placeholder-[#1A1A1A]/30 focus:outline-none focus:border-[#D62B2B]/40"/>
            ...
            {payMethod === 'efectivo' && (
              <div className="space-y-1.5">
                <input type="number" value={efectivo} onChange={e=>setEfectivo(e.target.value)} placeholder="Monto recibido" className="w-full bg-[#FAFAFA] border border-[#E5E5E5] rounded-xl px-3 py-2 text-[#1A1A1A] text-sm focus:outline-none focus:border-[#D62B2B]/40"/>
            ...
            <div className="flex justify-between font-display font-bold text-lg border-t border-[#E5E5E5] pt-2">
              ...
              <span className="text-[#D62B2B]">${total.toLocaleString()}</span>
            </div>

            <button onClick={confirmarVenta} disabled={!payMethod || (payMethod==='efectivo' && cambio < 0)}
              className="w-full bg-[#D62B2B] text-white font-bold py-3.5 rounded-xl hover:bg-[#B82222] transition-all disabled:opacity-40 disabled:cursor-not-allowed text-sm">
              ✓ Confirmar Venta
            </button>
          </div>
        )}
